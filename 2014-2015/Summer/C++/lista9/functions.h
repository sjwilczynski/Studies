...
template< typename D > std::map< std::string, unsigned int, D > 
nfrequencytable( const std::vector<std::string > & text, const  D& cmp )
{
    std::map< std::string, unsigned int, D > counter{cmp};
    for(auto& s : text)
    {
        ++ counter[s];
    }
...
template< typename C, typename D > std::unordered_map < std::string, unsigned int, C, D >
nhash_frequencytable( const std::vector< std::string >& text, const C& hash, const D& equal)
{
	std::unordered_map < std::string, unsigned int, C, D > counter{5,hash,equal};
	for(auto& s : text)
    	{
        	++ counter[s];
    	}
