...
polynomial<M> operator - (const polynomial<M>& pol1, const polynomial<M>& pol2)
{
    polynomial<M> p = pol1; //how is this possible if there is no copy constructor??
    return p -= pol2;
}
...
{
    polynomial<M> res;
    for( auto p = pol1. begin( ); p != pol1. end( ); ++ p )
    {
        for( auto q = pol2. begin( ); q != pol2. end( ); ++ q )
        {
            varchain v = (p->first)*(q->first);
            M val = (p->second)*(q->second);
            res[v] += val;
        }
    }
    return res;
